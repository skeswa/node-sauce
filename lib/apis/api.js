...
module.exports.OAuth2API = function(apiName, sauceConfigMap, apiConfigMap, extensionMap) {
    ...
    this.route = function(app) {
        ...
        var doSuccess = function() {
            try {
                if (res.session[authSuccessCallbackIdSessionKey]) callbackCache.pop(res.session[authSuccessCallbackIdSessionKey])();
                if (res.session[authGeneralCallbackIdSessionKey]) callbackCache.pop(res.session[authGeneralCallbackIdSessionKey])(null);
            } catch (e) {
        ...
        var doFailure = function(err) {
            try {
                if (res.session[authFailureCallbackIdSessionKey]) callbackCache.pop(res.session[authFailureCallbackIdSessionKey])(error);
                if (res.session[authGeneralCallbackIdSessionKey]) callbackCache.pop(res.session[authGeneralCallbackIdSessionKey])(error);
            } catch (e) {
                log.w("Keep in mind that you can't send a response from inside a sauce auth callback, because its magic. ;D.").
                throw e;
            }
