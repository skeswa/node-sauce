...
module.exports.OAuth2API = function(apiName, sauceConfigMap, apiConfigMap, extensionMap) {
    ...
    this.auth = function(app, req, res, returnUrl) {
        ...
        var chain = {
            success: function(callback) {
                ...
                if (res.session[authSuccessCallbackIdSessionKey]) callbackCache.pop(res.session[authSuccessCallbackIdSessionKey]);
                // Store this new callback id
                res.session[authSuccessCallbackIdSessionKey] = callbackId;
            ...
            failure: function(callback) {
                ...
                var callbackId = callbackCache.push(callback, 60000);
                // Get rid of old orphan callbacks
                if (res.session[authFailureCallbackIdSessionKey]) callbackCache.pop(res.session[authFailureCallbackIdSessionKey]);
                // Store this new callback id
                res.session[authFailureCallbackIdSessionKey] = callbackId;
            ...
            call: function(callback) {
                if (callback) {
                    ...
                    // Get rid of old orphan callbacks
                    if (res.session[authGeneralCallbackIdSessionKey]) callbackCache.pop(res.session[authGeneralCallbackIdSessionKey]);
                    // Store this new callback id
                    res.session[authGeneralCallbackIdSessionKey] = callbackId;
                }
                // Execute that redirect
                res.redirect(GOOGLE_OAUTH_AUTH_URL + "?" + qs.stringify(query));
            }
        };
    };
    // Creates a new client // TODO
    ...
    this.route = function(app) {
        ...
        var doSuccess = function(token) {
            try {
                if (res.session[authSuccessCallbackIdSessionKey]) {
                    var callback = callbackCache.pop(res.session[authSuccessCallbackIdSessionKey]);
                    if (callback) callback(token);
                }
                if (res.session[authGeneralCallbackIdSessionKey]) {
                    var callback = callbackCache.pop(res.session[authGeneralCallbackIdSessionKey]);
                    if (callback) callback(null, token);
                }
            } catch (e) {
                log.w("Keep in mind that you can't send a response from inside a sauce auth callback, because its magic. ;D.").
                throw e;
            }
        };
        var doFailure = function(err) {
            try {
                if (res.session[authFailureCallbackIdSessionKey]) {
                    var callback = callbackCache.pop(res.session[authFailureCallbackIdSessionKey]);
        ...
        if (!routed) {
            ...
            app.get(redirectRouteUrl, function(req, res) {
                ...
                if (!code) {
                    ...
                    doFailure(new ex.APICallFailureException(msg));
                    ...
                    request({
                        ...
                        if (error || response.statusCode > 299) {
                            ...
                            doSuccess(payload.access_token);
                        }
