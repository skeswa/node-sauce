...
module.exports.register = function(app, config, apiConfig) {
    if (!registered) {
        ...
        API_REDIRECT_URL = API_ROUTE_ROOT + "/google/redirect"

        log.d("Authentication redirect for 'google' API added: '" + API_REDIRECT_URL + "'.");
            ...
            if (!code) {
                ...
                request({
                    ...
                    if (error || response.statusCode > 200) {
                        ...
                        res.redirect(req.session.returnUrl ? req.session.returnUrl : "/");
                    }
                });
            };
        });
...
module.exports.makeClient = function(session) {
	...
	if(!session[SESSION_TOKEN_ID]) throw ex.APINotAuthorizedException("authorization token was null");
