...
var authHeader = function(args) {
    ...
    if (!underscore.isString(args.signatureMethod)) {
        ...
        headerMap["oauth_signature_method"] = "HMAC-SHA1";
    } else {
        headerMap["oauth_signature_method"] = args.signatureMethod;
    }
    // Build the signature
    headerMap["oauth_signature"] = signature(args.httpMethod || "POST", args.requestUrl, headerMap, headerMap["oauth_signature_method"], args.consumerSecret, args.tokenSecret);
...
module.exports = function(apiName, sauceConfigMap, apiConfigMap, extensionMap) {
    ...
    var authHeaderSessionKey = SAUCE_SESSION_KEY_PREFIX + camelCaseApiName + "AuthHeader";
            ...
            call: function(callback) {
                ...
                request({
                    ...
                    if (error || response.statusCode > 299) {
                        ...
                        var callback = null;
                        if (req.session[authFailureCallbackIdSessionKey]) callback = callbackCache.pop(req.session[authFailureCallbackIdSessionKey]);
                        if (callback) callback(error);
                        if (req.session[authGeneralCallbackIdSessionKey]) callback = callbackCache.pop(req.session[authGeneralCallbackIdSessionKey]);
                        if (callback) callback(error);
                        // Send back a 500
                        res.json(500, error);
                    } else {
