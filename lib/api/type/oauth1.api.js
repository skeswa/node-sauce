...
module.exports = function(apiName, sauceConfigMap, apiConfigMap, extensionMap) {
    ...
    this.request = function(session, method, url) {
        ...
        if (!session[authTokenSecretSessionKey]) throw apiEx.APINotAuthorizedException("'" + apiName + "' api secret authorization token was null.");
        assert.string(method, "method");
        assert.string(url, "url");
        // Make the client if it doesn't already exist
        if (!client) {
            client = new Client(requestUrl, function(config, method, url, params) {
                // Do parameter check
