...
var API_NAME = "dropbox";
var DEFAULT_API_ROOT = "/sauce/apis";

var DROPBOX_BASE_URL = "https://api.dropbox.com/1/"; //TODO: dropbox has 2 base urls (the other is https://api-content.dropbox.com/1)
var DROPBOX_OAUTH_AUTH_URL = "https://www.dropbox.com/1/oauth2/authorize";
var DROPBOX_OAUTH_TOKEN_URL = "https://api.dropbox.com/1/oauth2/token";
var SESSION_TOKEN_ID = "dropboxApiToken";
            ...
            if (!code) {
                ...
                request({
                    ...
                    if (error || response.statusCode > 299) {
                        if (!error) error = new ex.APICallFailureException("API call to URL '" + DROPBOX_OAUTH_TOKEN_URL + "' failed with status code " + response.statusCode + ". The body was as follows: \n" + body);
...
var auth = function(app, req, res, returnUrl) {
    if (!authed(req.session)) {
        ...
        res.redirect(DROPBOX_OAUTH_AUTH_URL + '?' + queryString);
    } else {
...
var makeClient = function(session) {
    ...
    return new Client(DROPBOX_BASE_URL, session[SESSION_TOKEN_ID], {
