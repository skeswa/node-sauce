 ...
 */
var OAuth1Client = function(baseUrl, authHeaderFactory) {
    ...
    this.request = function(method, url) {
        ...
        var chainObj = {
            set: function(_key, _value) {
                if (underscore.isObject(_key)) {
                    // We got a map instead of a key-value pair
                    for (var paramKey in _key) {
                        config[paramKey] = _key[paramKey];
                    }
                } else if (underscore.isString(_key)) {
                    config[_key] = _value;
                } else {
                    throw new ex.IllegalArgumentException("The 'key' parameter must be a string or an object.");
                }
                return chainObj;
            },
            ...
            call: function(_callback) {
                ...
                var headers = {
                    Authorization: authHeaderFactory(config, method, options["url"], payload)
                };
